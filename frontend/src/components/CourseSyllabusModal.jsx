...
export default function CourseSyllabusModal({ course, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div 
        className="bg-surface border border-slate-700 rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex justify-between items-start p-6 border-b border-slate-700 bg-slate-800/40">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="bg-primary/20 text-primary text-xs font-bold px-2.5 py-1 rounded-md">{syllabusData.yearTerm}</span>
              <span className="bg-slate-700 text-slate-300 text-xs font-bold px-2.5 py-1 rounded-md">{course.code}</span>
              <span className="bg-slate-700 text-slate-300 text-xs font-bold px-2.5 py-1 rounded-md">{syllabusData.type}</span>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
          ...
          <section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700">
                <div className="text-slate-400 text-xs mb-1 flex items-center gap-1"><User size={12}/> 담당교수</div>
                <div className="font-semibold text-slate-200">{course.professor}</div>
              </div>
              <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700">
                <div className="text-slate-400 text-xs mb-1 flex items-center gap-1"><Mail size={12}/> 이메일</div>
                <div className="font-semibold text-slate-200 text-sm truncate">{syllabusData.email}</div>
              </div>
              <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700">
                <div className="text-slate-400 text-xs mb-1">학점 (이론/실습)</div>
                <div className="font-semibold text-slate-200">{course.credits}학점 ({syllabusData.creditInfo})</div>
              </div>
              <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700">
                <div className="text-slate-400 text-xs mb-1 flex items-center gap-1"><Phone size={12}/> 연구실 / 연락처</div>
                <div className="font-semibold text-slate-200 text-sm">{syllabusData.lab} / {syllabusData.tel}</div>
              </div>
            </div>
          </section>
          ...
          <section>
            ...
            <div className="bg-slate-800/30 rounded-xl border border-slate-700 overflow-hidden text-sm">
              <div className="grid grid-cols-1 md:grid-cols-4 border-b border-slate-700">
                <div className="bg-slate-800/80 p-3 text-slate-400 font-medium">평가방법</div>
                <div className="p-3 text-slate-200">{syllabusData.evalMethod}</div>
                <div className="bg-slate-800/80 p-3 text-slate-400 font-medium border-t md:border-t-0 md:border-l border-slate-700">평가요소</div>
                <div className="p-3 text-slate-200">{syllabusData.evalElements}</div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-4 border-b border-slate-700">
                <div className="bg-slate-800/80 p-3 text-slate-400 font-medium">교재</div>
                <div className="p-3 text-slate-200 md:col-span-3">{syllabusData.textbook}</div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-4 border-b border-slate-700">
                <div className="bg-slate-800/80 p-3 text-slate-400 font-medium">시험/과제</div>
                <div className="p-3 text-slate-200 md:col-span-3 whitespace-pre-wrap">{syllabusData.exams}</div>
              </div>
            </div>
          </section>
          ...
          <section>
            ...
            <div className="space-y-4">
              <div className="bg-slate-800/30 p-5 rounded-xl border border-slate-700">
                <h4 className="text-sm font-bold text-slate-400 mb-2">수업 개요</h4>
                <p className="text-sm text-slate-200 leading-relaxed whitespace-pre-wrap">{syllabusData.overview}</p>
              </div>
              <div className="bg-slate-800/30 p-5 rounded-xl border border-slate-700">
                <h4 className="text-sm font-bold text-slate-400 mb-3">수업 목표</h4>
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-slate-700/50 bg-slate-800/20">
                  {syllabusData.weeklyPlan.map((plan) => (
                    <tr key={plan.week} className="hover:bg-slate-800/50 transition-colors">
                      <td className="px-4 py-3 text-center font-bold text-slate-300">{plan.week}주차</td>
                      <td className="px-4 py-3 text-slate-200">{plan.range}</td>
                      <td className="px-4 py-3 text-center text-slate-400 text-xs">{plan.method}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
